...
function formatError(error: z.ZodError): string {
  return `AgencyPersonnelList is malformed at ${firstIssuePath(error)}.`;
}
...
export const schema = z
  .object({
    ...
    kind: z.literal("AgencyPersonnelList"),
...
async function readAgencyPersonnelList(
  pathOrRef: string | EnvelopeReadRef,
  options: EnvelopeReadOptions = {},
  ...
  const envelope = parseAgencyPersonnelList(document);
  if (
    options.expectedNamespace !== undefined &&
    envelope.metadata.namespace !== options.expectedNamespace
  ) {
    throw new Error(
      `AgencyPersonnelList namespace ${envelope.metadata.namespace} does not match expected namespace ${options.expectedNamespace}: ${ref.filePath}`,
    );
  }
  return envelope;
}

async function writeAgencyPersonnelList(
  directory: string,
  envelope: AgencyPersonnelListEnvelope,
): Promise<{ path: string }> {
  const parsed = parseAgencyPersonnelList(envelope);
  const filePath = yamlResourcePath(directory, parsed);
  await writeYamlDocumentFile(filePath, parsed);
  return { path: filePath };
}

export const AgencyPersonnelList = {
  kind: "AgencyPersonnelList",
  schema,
  new: newAgencyPersonnelList,
  read: readAgencyPersonnelList,
  write: writeAgencyPersonnelList,
};
