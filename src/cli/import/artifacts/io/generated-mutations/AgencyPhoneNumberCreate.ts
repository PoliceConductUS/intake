...
function formatError(error: z.ZodError): string {
  return `AgencyPhoneNumberCreate is malformed at ${firstIssuePath(error)}.`;
}
...
export const schema = z
  .object({
    ...
    kind: z.literal("AgencyPhoneNumberCreate"),
...
async function readAgencyPhoneNumberCreate(
  pathOrRef: string | EnvelopeReadRef,
  options: EnvelopeReadOptions = {},
  ...
  const envelope = parseAgencyPhoneNumberCreate(document);
  if (
    options.expectedNamespace !== undefined &&
    envelope.metadata.namespace !== options.expectedNamespace
  ) {
    throw new Error(
      `AgencyPhoneNumberCreate namespace ${envelope.metadata.namespace} does not match expected namespace ${options.expectedNamespace}: ${ref.filePath}`,
    );
  }
  return envelope;
}

async function writeAgencyPhoneNumberCreate(
  directory: string,
  envelope: AgencyPhoneNumberCreateEnvelope,
): Promise<{ path: string }> {
  const parsed = parseAgencyPhoneNumberCreate(envelope);
  const filePath = yamlResourcePath(directory, parsed);
  await writeYamlDocumentFile(filePath, parsed);
  return { path: filePath };
}

export const AgencyPhoneNumberCreate = {
  kind: "AgencyPhoneNumberCreate",
  schema,
  new: newAgencyPhoneNumberCreate,
  read: readAgencyPhoneNumberCreate,
  write: writeAgencyPhoneNumberCreate,
};
