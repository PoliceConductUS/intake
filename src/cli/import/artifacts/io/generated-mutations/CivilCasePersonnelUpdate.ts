...
function resolveReadPath(
  ...
  if (
    ...
    throw new Error(
      `Relative ${ref.kind ?? "CivilCasePersonnelUpdate"} ref requires relativeTo.`,
    );
  }
  ...
  if (relativePath.startsWith("..") || path.isAbsolute(relativePath)) {
    throw new Error(
      `${ref.kind ?? "CivilCasePersonnelUpdate"} ref.path escapes its directory: ${ref.path}`,
    );
  }
...
const fieldSchemas = (CivilCasePersonnelSpec as z.ZodObject<z.ZodRawShape>)
  .shape;
...
export type CivilCasePersonnelUpdateInput = Omit<
  CivilCasePersonnelUpdateEnvelope,
  "apiVersion" | "kind"
>;

function parseCivilCasePersonnelUpdate(
  value: unknown,
): CivilCasePersonnelUpdateEnvelope {
...
function newCivilCasePersonnelUpdate(
  input: CivilCasePersonnelUpdateInput,
): CivilCasePersonnelUpdateEnvelope {
...
async function readCivilCasePersonnelUpdate(
  ...
  if (ref.kind !== undefined && ref.kind !== "CivilCasePersonnelUpdate") {
    throw new Error(
      `CivilCasePersonnelUpdate ref.kind ${ref.kind} does not match expected kind CivilCasePersonnelUpdate: ${ref.filePath}`,
    );
  }
  ...
  if (ref.sha256 !== undefined && yamlDigest(contents) !== ref.sha256) {
    throw new Error(
      `CivilCasePersonnelUpdate sha256 mismatch: ${ref.filePath}`,
    );
  }
