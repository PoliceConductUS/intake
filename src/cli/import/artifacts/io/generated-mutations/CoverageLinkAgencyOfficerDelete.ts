...
function formatError(error: z.ZodError): string {
  return `CoverageLinkAgencyOfficerDelete is malformed at ${firstIssuePath(error)}.`;
}
...
export const schema = z
  .object({
    ...
    kind: z.literal("CoverageLinkAgencyOfficerDelete"),
...
async function readCoverageLinkAgencyOfficerDelete(
  pathOrRef: string | EnvelopeReadRef,
  options: EnvelopeReadOptions = {},
  ...
  if (
    ...
    throw new Error(
      `CoverageLinkAgencyOfficerDelete namespace ${envelope.metadata.namespace} does not match expected namespace ${options.expectedNamespace}: ${ref.filePath}`,
    );
  }
  return envelope;
}

async function writeCoverageLinkAgencyOfficerDelete(
  directory: string,
  envelope: CoverageLinkAgencyOfficerDeleteEnvelope,
): Promise<{ path: string }> {
  const parsed = parseCoverageLinkAgencyOfficerDelete(envelope);
  const filePath = yamlResourcePath(directory, parsed);
  await writeYamlDocumentFile(filePath, parsed);
  return { path: filePath };
}

export const CoverageLinkAgencyOfficerDelete = {
  kind: "CoverageLinkAgencyOfficerDelete",
  schema,
  new: newCoverageLinkAgencyOfficerDelete,
  read: readCoverageLinkAgencyOfficerDelete,
  write: writeCoverageLinkAgencyOfficerDelete,
};
