...
function formatError(error: z.ZodError): string {
  return `CoverageLinkCreate is malformed at ${firstIssuePath(error)}.`;
}
...
export const schema = z
  .object({
    ...
    kind: z.literal("CoverageLinkCreate"),
...
async function readCoverageLinkCreate(
  pathOrRef: string | EnvelopeReadRef,
  options: EnvelopeReadOptions = {},
  ...
  const envelope = parseCoverageLinkCreate(document);
  if (
    options.expectedNamespace !== undefined &&
    envelope.metadata.namespace !== options.expectedNamespace
  ) {
    throw new Error(
      `CoverageLinkCreate namespace ${envelope.metadata.namespace} does not match expected namespace ${options.expectedNamespace}: ${ref.filePath}`,
    );
  }
  return envelope;
}

async function writeCoverageLinkCreate(
  directory: string,
  envelope: CoverageLinkCreateEnvelope,
): Promise<{ path: string }> {
  const parsed = parseCoverageLinkCreate(envelope);
  const filePath = yamlResourcePath(directory, parsed);
  await writeYamlDocumentFile(filePath, parsed);
  return { path: filePath };
}

export const CoverageLinkCreate = {
  kind: "CoverageLinkCreate",
  schema,
  new: newCoverageLinkCreate,
  read: readCoverageLinkCreate,
  write: writeCoverageLinkCreate,
};
