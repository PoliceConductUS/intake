...
function formatError(error: z.ZodError): string {
  return `CoverageLinkDelete is malformed at ${firstIssuePath(error)}.`;
}
...
export const schema = z
  .object({
    ...
    kind: z.literal("CoverageLinkDelete"),
...
async function readCoverageLinkDelete(
  pathOrRef: string | EnvelopeReadRef,
  options: EnvelopeReadOptions = {},
  ...
  const envelope = parseCoverageLinkDelete(document);
  if (
    options.expectedNamespace !== undefined &&
    envelope.metadata.namespace !== options.expectedNamespace
  ) {
    throw new Error(
      `CoverageLinkDelete namespace ${envelope.metadata.namespace} does not match expected namespace ${options.expectedNamespace}: ${ref.filePath}`,
    );
  }
  return envelope;
}

async function writeCoverageLinkDelete(
  directory: string,
  envelope: CoverageLinkDeleteEnvelope,
): Promise<{ path: string }> {
  const parsed = parseCoverageLinkDelete(envelope);
  const filePath = yamlResourcePath(directory, parsed);
  await writeYamlDocumentFile(filePath, parsed);
  return { path: filePath };
}

export const CoverageLinkDelete = {
  kind: "CoverageLinkDelete",
  schema,
  new: newCoverageLinkDelete,
  read: readCoverageLinkDelete,
  write: writeCoverageLinkDelete,
};
