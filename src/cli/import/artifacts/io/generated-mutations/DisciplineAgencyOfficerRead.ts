...
function resolveReadPath(
  ...
  if (
    ...
    throw new Error(
      `Relative ${ref.kind ?? "DisciplineAgencyOfficerRead"} ref requires relativeTo.`,
    );
  }
  ...
  if (relativePath.startsWith("..") || path.isAbsolute(relativePath)) {
    throw new Error(
      `${ref.kind ?? "DisciplineAgencyOfficerRead"} ref.path escapes its directory: ${ref.path}`,
    );
  }
...
export type DisciplineAgencyOfficerReadInput = Omit<
  DisciplineAgencyOfficerReadEnvelope,
  "apiVersion" | "kind"
>;

function parseDisciplineAgencyOfficerRead(
  value: unknown,
): DisciplineAgencyOfficerReadEnvelope {
...
function newDisciplineAgencyOfficerRead(
  input: DisciplineAgencyOfficerReadInput,
): DisciplineAgencyOfficerReadEnvelope {
...
async function readDisciplineAgencyOfficerRead(
  ...
  if (ref.kind !== undefined && ref.kind !== "DisciplineAgencyOfficerRead") {
    throw new Error(
      `DisciplineAgencyOfficerRead ref.kind ${ref.kind} does not match expected kind DisciplineAgencyOfficerRead: ${ref.filePath}`,
    );
  }
  ...
  if (ref.sha256 !== undefined && yamlDigest(contents) !== ref.sha256) {
    throw new Error(
      `DisciplineAgencyOfficerRead sha256 mismatch: ${ref.filePath}`,
    );
  }
