...
function resolveReadPath(
  ...
  if (
    ...
    throw new Error(
      `Relative ${ref.kind ?? "DisciplineAgencyPersonnelRead"} ref requires relativeTo.`,
    );
  }
  ...
  if (relativePath.startsWith("..") || path.isAbsolute(relativePath)) {
    throw new Error(
      `${ref.kind ?? "DisciplineAgencyPersonnelRead"} ref.path escapes its directory: ${ref.path}`,
    );
  }
...
export type DisciplineAgencyPersonnelReadInput = Omit<
  DisciplineAgencyPersonnelReadEnvelope,
  "apiVersion" | "kind"
>;

function parseDisciplineAgencyPersonnelRead(
  value: unknown,
): DisciplineAgencyPersonnelReadEnvelope {
...
function newDisciplineAgencyPersonnelRead(
  input: DisciplineAgencyPersonnelReadInput,
): DisciplineAgencyPersonnelReadEnvelope {
...
async function readDisciplineAgencyPersonnelRead(
  ...
  if (ref.kind !== undefined && ref.kind !== "DisciplineAgencyPersonnelRead") {
    throw new Error(
      `DisciplineAgencyPersonnelRead ref.kind ${ref.kind} does not match expected kind DisciplineAgencyPersonnelRead: ${ref.filePath}`,
    );
  }
  ...
  if (ref.sha256 !== undefined && yamlDigest(contents) !== ref.sha256) {
    throw new Error(
      `DisciplineAgencyPersonnelRead sha256 mismatch: ${ref.filePath}`,
    );
  }
