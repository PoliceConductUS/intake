...
function formatError(error: z.ZodError): string {
  return `DisciplineCreate is malformed at ${firstIssuePath(error)}.`;
}
...
export const schema = z
  .object({
    ...
    kind: z.literal("DisciplineCreate"),
...
async function readDisciplineCreate(
  pathOrRef: string | EnvelopeReadRef,
  options: EnvelopeReadOptions = {},
  ...
  const envelope = parseDisciplineCreate(document);
  if (
    options.expectedNamespace !== undefined &&
    envelope.metadata.namespace !== options.expectedNamespace
  ) {
    throw new Error(
      `DisciplineCreate namespace ${envelope.metadata.namespace} does not match expected namespace ${options.expectedNamespace}: ${ref.filePath}`,
    );
  }
  return envelope;
}

async function writeDisciplineCreate(
  directory: string,
  envelope: DisciplineCreateEnvelope,
): Promise<{ path: string }> {
  const parsed = parseDisciplineCreate(envelope);
...
export const DisciplineCreate = {
  kind: "DisciplineCreate",
  schema,
  new: newDisciplineCreate,
  read: readDisciplineCreate,
  write: writeDisciplineCreate,
};
