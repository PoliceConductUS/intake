...
function formatError(error: z.ZodError): string {
  return `LocationPathCreate is malformed at ${firstIssuePath(error)}.`;
}
...
export const schema = z
  .object({
    ...
    kind: z.literal("LocationPathCreate"),
...
async function readLocationPathCreate(
  pathOrRef: string | EnvelopeReadRef,
  options: EnvelopeReadOptions = {},
  ...
  const envelope = parseLocationPathCreate(document);
  if (
    options.expectedNamespace !== undefined &&
    envelope.metadata.namespace !== options.expectedNamespace
  ) {
    throw new Error(
      `LocationPathCreate namespace ${envelope.metadata.namespace} does not match expected namespace ${options.expectedNamespace}: ${ref.filePath}`,
    );
  }
  return envelope;
}

async function writeLocationPathCreate(
  directory: string,
  envelope: LocationPathCreateEnvelope,
): Promise<{ path: string }> {
  const parsed = parseLocationPathCreate(envelope);
  const filePath = yamlResourcePath(directory, parsed);
  await writeYamlDocumentFile(filePath, parsed);
  return { path: filePath };
}

export const LocationPathCreate = {
  kind: "LocationPathCreate",
  schema,
  new: newLocationPathCreate,
  read: readLocationPathCreate,
  write: writeLocationPathCreate,
};
