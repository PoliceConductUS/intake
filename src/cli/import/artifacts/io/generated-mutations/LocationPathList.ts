...
function formatError(error: z.ZodError): string {
  return `LocationPathList is malformed at ${firstIssuePath(error)}.`;
}
...
export const schema = z
  .object({
    ...
    kind: z.literal("LocationPathList"),
...
async function readLocationPathList(
  pathOrRef: string | EnvelopeReadRef,
  options: EnvelopeReadOptions = {},
  ...
  const envelope = parseLocationPathList(document);
  if (
    options.expectedNamespace !== undefined &&
    envelope.metadata.namespace !== options.expectedNamespace
  ) {
    throw new Error(
      `LocationPathList namespace ${envelope.metadata.namespace} does not match expected namespace ${options.expectedNamespace}: ${ref.filePath}`,
    );
  }
  return envelope;
}

async function writeLocationPathList(
  directory: string,
  envelope: LocationPathListEnvelope,
): Promise<{ path: string }> {
  const parsed = parseLocationPathList(envelope);
  const filePath = yamlResourcePath(directory, parsed);
  await writeYamlDocumentFile(filePath, parsed);
  return { path: filePath };
}

export const LocationPathList = {
  kind: "LocationPathList",
  schema,
  new: newLocationPathList,
  read: readLocationPathList,
  write: writeLocationPathList,
};
