...
function formatError(error: z.ZodError): string {
  return `LocationPathUpdate is malformed at ${firstIssuePath(error)}.`;
}
  ...
  (operation, context) => {
    if (operation.path.includes(".")) {
      context.addIssue({
        code: "custom",
        path: ["path"],
        message: "must be a LocationPath field",
      });
      return;
    }
...
export const schema = z
  .object({
    ...
    kind: z.literal("LocationPathUpdate"),
...
async function readLocationPathUpdate(
  pathOrRef: string | EnvelopeReadRef,
  options: EnvelopeReadOptions = {},
  ...
  const envelope = parseLocationPathUpdate(document);
  if (
    options.expectedNamespace !== undefined &&
    envelope.metadata.namespace !== options.expectedNamespace
  ) {
    throw new Error(
      `LocationPathUpdate namespace ${envelope.metadata.namespace} does not match expected namespace ${options.expectedNamespace}: ${ref.filePath}`,
    );
  }
  return envelope;
}

async function writeLocationPathUpdate(
  directory: string,
  envelope: LocationPathUpdateEnvelope,
): Promise<{ path: string }> {
  const parsed = parseLocationPathUpdate(envelope);
  const filePath = yamlResourcePath(directory, parsed);
  await writeYamlDocumentFile(filePath, parsed);
  return { path: filePath };
}

export const LocationPathUpdate = {
  kind: "LocationPathUpdate",
  schema,
  new: newLocationPathUpdate,
  read: readLocationPathUpdate,
  write: writeLocationPathUpdate,
};
