...
function formatError(error: z.ZodError): string {
  return `PersonnelDelete is malformed at ${firstIssuePath(error)}.`;
}
...
export const schema = z
  .object({
    ...
    kind: z.literal("PersonnelDelete"),
...
async function readPersonnelDelete(
  pathOrRef: string | EnvelopeReadRef,
  options: EnvelopeReadOptions = {},
  ...
  const envelope = parsePersonnelDelete(document);
  if (
    options.expectedNamespace !== undefined &&
    envelope.metadata.namespace !== options.expectedNamespace
  ) {
    throw new Error(
      `PersonnelDelete namespace ${envelope.metadata.namespace} does not match expected namespace ${options.expectedNamespace}: ${ref.filePath}`,
    );
  }
  return envelope;
}

async function writePersonnelDelete(
  directory: string,
  envelope: PersonnelDeleteEnvelope,
): Promise<{ path: string }> {
  const parsed = parsePersonnelDelete(envelope);
  const filePath = yamlResourcePath(directory, parsed);
  await writeYamlDocumentFile(filePath, parsed);
  return { path: filePath };
}

export const PersonnelDelete = {
  kind: "PersonnelDelete",
  schema,
  new: newPersonnelDelete,
  read: readPersonnelDelete,
  write: writePersonnelDelete,
};
