...
// An intake-owned resolver injected into a source's transform phase (ADR 0023). The
...
export type TransformDataContext = {
...
export type TransformDeps = {
  ...
  data?: TransformDataContext;
  logger?: { info: (message: string) => void };
};
export type SourceTransform = (deps: TransformDeps) => Promise<SourceManifest>;
...
// so the deterministic `transform` (produce) phase can then read them. Network and
// non-determinism live here, never in `transform`.
