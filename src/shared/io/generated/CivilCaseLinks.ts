...
function formatError(error: z.ZodError): string {
  return `CivilCaseLinks is malformed at ${firstIssuePath(error)}.`;
}
...
function recordSpecSchema(action?: "PUT" | "PATCH" | "POST") {
  return action === "PATCH" ? CivilCaseLinkSpec.partial() : CivilCaseLinkSpec;
}
...
> & {
  spec: Omit<CivilCaseLinksEnvelope["spec"], "records"> & {
    records: Record<
      string,
      { metadata: RecordMetadata; spec: z.infer<typeof CivilCaseLinkSpec> }
    >;
  };
};
...
function parseCivilCaseLinks(value: unknown): CivilCaseLinksEnvelope {
...
function newCivilCaseLinks(input: CivilCaseLinksInput): CivilCaseLinksEnvelope {
  return parseCivilCaseLinks({
...
): z.infer<typeof CivilCaseLinkSpec> {
  const result = recordSpecSchema(action).safeParse(value);
  if (!result.success) {
    throw new Error(
      `CivilCaseLinks record ${recordKey} is malformed at ${firstIssuePath(result.error)}: ${artifactPath}`,
    );
  }
  return result.data as z.infer<typeof CivilCaseLinkSpec>;
}
...
): Promise<CivilCaseLinksEnvelope>;
async function readCivilCaseLinks(
  filePath: string,
  options?: EnvelopeReadOptions & {
    expectedKind?: "CivilCaseLinks";
...
): Promise<CivilCaseLinksResolvedEnvelope>;
async function readCivilCaseLinks(
  filePath: string,
  options: EnvelopeReadOptions & {
    expectedKind?: "CivilCaseLinks";
  ...
  if (
    ...
    throw new Error(
      `CivilCaseLinks namespace ${artifact.metadata.namespace} does not match expected namespace ${options.expectedNamespace}: ${filePath}`,
    );
  }
  if (options.raw === true) {
    return artifact;
...
async function writeCivilCaseLinks(
  directory: string,
  envelope: CivilCaseLinksInput,
  options: ImportArtifactWriteOptions = {},
): Promise<{ path: string; sha256: string }> {
  let artifact = newCivilCaseLinks(envelope);
...
export const CivilCaseLinks = {
  kind: "CivilCaseLinks",
  schema,
  new: newCivilCaseLinks,
