...
function recordSpecSchema(action?: "PUT" | "PATCH" | "POST") {
  return action === "PATCH" ? LicenseActionSpec.partial() : LicenseActionSpec;
}
...
> & {
  spec: Omit<LicenseActionsEnvelope["spec"], "records"> & {
    records: Record<
      string,
      { metadata: RecordMetadata; spec: z.infer<typeof LicenseActionSpec> }
    >;
  };
};
...
function validateRecord(
  ...
  return result.data as z.infer<typeof LicenseActionSpec>;
}
  ...
  if (options.externalizeRecords === true) {
    ...
    for (const [recordKey, recordItem] of Object.entries(
      ...
      const record = newLicenseAction({
        ...
        spec: recordItem.spec as z.infer<typeof LicenseActionSpec>,
      });
