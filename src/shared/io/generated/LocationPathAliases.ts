...
> & {
  spec: Omit<LocationPathAliasesEnvelope["spec"], "records"> & {
    records: Record<
      string,
      { metadata: RecordMetadata; spec: z.infer<typeof LocationPathAliasSpec> }
    >;
  };
};
...
function validateRecord(
  ...
  return result.data as z.infer<typeof LocationPathAliasSpec>;
}
  ...
  if (options.externalizeRecords === true) {
    ...
    for (const [recordKey, recordItem] of Object.entries(
      ...
      const record = newLocationPathAlias({
        ...
        spec: recordItem.spec as z.infer<typeof LocationPathAliasSpec>,
      });
