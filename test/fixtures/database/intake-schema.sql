...
CREATE TABLE public.location_path (
    ...
    created_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL,
    updated_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL,
    CONSTRAINT location_path_administrative_area_name_not_blank CHECK (((administrative_area_name IS NULL) OR (char_length(btrim(administrative_area_name)) > 0))),
    ...
    CONSTRAINT location_path_place_name_not_blank CHECK (((place_name IS NULL) OR (char_length(btrim(place_name)) > 0))),
    CONSTRAINT location_path_place_slug_not_blank CHECK (((place_slug IS NULL) OR (char_length(btrim(place_slug)) > 0))),
    CONSTRAINT location_path_state_or_territory_name_not_blank CHECK ((char_length(btrim(state_or_territory_name)) > 0)),
